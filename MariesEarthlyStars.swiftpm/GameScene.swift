...
class GameScene: SKScene {
    ...
    var martelo, pechblenda, mangueira, petri, erlen, dropper, retangulo: SKSpriteNode?
    ...
    let pechSize = CGSize(width: 380, height: 380)
    ...
    override func didMove(to view: SKView) {
        ...
        let background = SKSpriteNode(imageNamed: "game-screen-1")
    ...
    func positionThings() {
        ...
        pechblenda = SKSpriteNode(imageNamed: "pechblenda-folhas")
        pechblenda?.size = pechSize
        pechblenda?.position = CGPoint(x: self.frame.midX, y: self.frame.midY)
        addChild(pechblenda!)
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if let touch = touches.first {
            ...
            if (touchedNode.name == "journalButton" && !isJournalOpen) {
                ...
                let popup = SKSpriteNode(imageNamed: "journal-1")
    ...
    override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if pechblenda!.color == .red {
            let secondScene = SecondGameScene(size: self.size)
            secondScene.scaleMode = self.scaleMode
            let transition = SKTransition.fade(withDuration: 0.2)
    ...
    override func update(_ currentTime: TimeInterval) {
        if pechblenda!.position.x+80 <= martelo!.position.x+200 && pechblenda!.position.x-80 >= martelo!.position.x-200 && pechblenda!.position.y+80 <= martelo!.position.y+200 && pechblenda!.position.y-80 >= martelo!.position.y-200 {
            
            pechblenda!.color = UIColor.red
        }
    }
}
