//
//  FourthGameScene.swift
//  MariesEarthlyStars
//
//  Created by Bianca Nathally Bezerra de Lima on 17/04/23.
//
...
class FourthGameScene: SKScene {
    ...
    var erlenLiquido, menuButton: SKSpriteNode!
    var martelo, mangueira, petri, pote, dropper, retangulo, journal, iconJournal: SKSpriteNode?
    ...
    let erlenLiquidoSize = CGSize(width: 380, height: 380)
    ...
    override func didMove(to view: SKView) {
        ...
        let background = SKSpriteNode(imageNamed: "game-screen-4")
    ...
    func positionThings() {
        ...
        erlenLiquido = SKSpriteNode(imageNamed: "erlen-liquido")
        erlenLiquido?.size = erlenLiquidoSize
        erlenLiquido.position = CGPoint(x: frame.midX, y: frame.midY)
        addChild(erlenLiquido)
        ...
        petri?.position = CGPoint(x: self.frame.midX-120, y: self.frame.midY-380)
        addChild(petri!)
        ...
        pote?.name = "pote"
        pote?.size = poteSize
        pote?.position = CGPoint(x: self.frame.midX+80, y: self.frame.midY-380)
        addChild(pote!)
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if let touch = touches.first {
            ...
            if (touchedNode.name == "journalButton" && !isJournalOpen) {
                ...
                let popup = SKSpriteNode(imageNamed: "journal-4")
    ...
    override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if erlenLiquido!.color == .red {
            let fourthScene = FifthGameScene(size: self.size)
            fourthScene.scaleMode = self.scaleMode
            let transition = SKTransition.fade(withDuration: 0.2)
            self.view?.presentScene(fourthScene, transition: transition)
            mangueira?.position = CGPoint(x: self.frame.midX-200, y: self.frame.midY-200)
        }
    }
    
    override func update(_ currentTime: TimeInterval) {
        if erlenLiquido!.position.x+80 <= mangueira!.position.x+200 && erlenLiquido!.position.x-80 >= mangueira!.position.x-200 && erlenLiquido!.position.y+80 <= mangueira!.position.y+200 && erlenLiquido!.position.y-80 >= mangueira!.position.y-200 {
            
            erlenLiquido!.color = UIColor.red
        }
    }
}
